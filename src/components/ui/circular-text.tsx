...
const getRotationTransition = (duration: number, loop = true) => ({
  duration: duration,
  ...
  const handleHoverStart = () => {
    ...
    if (shouldPause) {
      controls.start({
        ...
        scale: 1,
        transition: {
          rotate: { type: "spring" as const, damping: 20, stiffness: 300, duration: 0.2 },
      ...
      controls.start({
        rotate: [currentRotation, currentRotation + 360],
        scale: newScale,
        transition: getTransition(newSpinDuration),
      });
    }
  };
